...
TEXT_CLICK_X = int(os.getenv("TEXT_CLICK_X", "800"))
TEXT_CLICK_Y = int(os.getenv("TEXT_CLICK_Y", "500"))
TEXT_KEY_INTERVAL = float(os.getenv("TEXT_KEY_INTERVAL", "0.02"))
TEXT_CLICK_WINDOW_TITLE = os.getenv("TEXT_CLICK_WINDOW_TITLE", "Excalidraw")
TEXT_CLICK_WINDOW_OFFSET_X = int(os.getenv("TEXT_CLICK_WINDOW_OFFSET_X", "0"))
TEXT_CLICK_WINDOW_OFFSET_Y = int(os.getenv("TEXT_CLICK_WINDOW_OFFSET_Y", "0"))
...
async def main():
    ...
    try:
        ...
        server_params = StdioServerParameters(
            command="python",
            args=["example2.py"],
            cwd="/Users/panchani/IdeaProjects/experiments/eagv2/session6"
        )
...
Your entire response must be a single line starting with either FUNCTION_CALL: or FINAL_ANSWER: """
                ...
                while iteration < max_iterations:
                    ...
                    if response_text.startswith("FUNCTION_CALL:"):
                        ...
                        result = await session.call_tool("open_excalidraw")
                        print(result.content[0].text)

                        # Wait for Excalidraw to fully load
                        await asyncio.sleep(2)

                        final_text = response_text.replace("FINAL_ANSWER: ", "")

                        try:
                            result = await session.call_tool(
                                "add_text_in_excalidraw_pyauto",
                                arguments={
                                    "text": final_text,
                                    "x": TEXT_CLICK_X,
                                    "y": TEXT_CLICK_Y,
                                    "interval": TEXT_KEY_INTERVAL,
                                    "window_title": TEXT_CLICK_WINDOW_TITLE,
                                    "window_offset_x": TEXT_CLICK_WINDOW_OFFSET_X,
                                    "window_offset_y": TEXT_CLICK_WINDOW_OFFSET_Y,
                                }
                            )
                        except Exception as e:
                            print(f"pyautogui path failed, falling back to AppleScript: {e}")
                            result = await session.call_tool(
                                "add_text_in_excalidraw",
                                arguments={
                                    "text": final_text
                                }
                            )
